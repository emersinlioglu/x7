...
class ModelModuleProductOptionImagePro extends Model {
  ...
  public function addOptionImagesToProductImages($product_images_old, $product_id, &$product_images, $popup_width=0, $popup_height=0) {
    ...
    foreach ($product_images as &$result) {
      ...
      if ($popup_width == 0 || $popup_height == 0) {
        $result['popup'] = $this->image_resize($result['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
      } else {
        $result['popup'] = $this->image_resize($result['image'], $popup_width, $popup_height);
      }
      $result['thumb'] = $this->image_resize($result['image'], $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'));
      $result['main'] = $this->image_resize($result['image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));
  ...
  public function image_resize($image, $width, $height) {
    ...
    if ( $this->config->get('config_template') == 'maxstore' && method_exists($this->model_tool_image, 'cropsize') ) {
  ...
  public function getCategoryImages($product_id, $module_setting) {
    ...
    if ( $this->config->get('config_template') == "marketshop" && $module_setting && isset($module_setting['name']) && $module_setting['name'] == 'bestseller' ) {
