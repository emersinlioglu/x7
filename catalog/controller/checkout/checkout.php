...
class ControllerCheckoutCheckout extends Controller {
	public function index() {
		...
		foreach ($products as $product) {
			...
			if (stripos($product['name'], 'prepaid') !== false) {
				$isPrepaid = true;
			} else {
				$isPrepaid = false;
				if (stripos($product['name'], 'vodafone') !== false) {
					$vertragsarten['vodafone'] = 1;
				}
				if (stripos($product['name'], 'mobilcom') !== false) {
					$vertragsarten['mobilcom'] = 1;
				} else if (stripos($product['name'], 'o2') !== false) {
					$vertragsarten['o2'] = 1;
				}
			}
